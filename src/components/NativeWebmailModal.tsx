    ...
    try {
      ...
      const response = await fetch('/api/webmail-send', {
        ...
        body: JSON.stringify({
          ...
          html: `<div style="font-family: Arial, sans-serif; font-size: 13px; color: #073642; line-height: 1.6;">${fullBodyWithSignature.replace(/\n/g, '<br/>')}</div>`
        })
      });
      ...
      <div className="bg-[#FDF6E3] w-full max-w-2xl rounded-3xl p-6 border border-[#D6D1B1] shadow-2xl space-y-4 my-8 text-[#073642]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-lg font-bold text-[#073642]">Official Webmail Composer</h3>
        ...
        {sendSuccess ? (
          <div className="py-12 flex flex-col items-center justify-center space-y-3 text-center">
            ...
            <h3 className="text-xl font-bold text-[#073642]">Email Dispatched via Webmail SMTP!</h3>
            <p className="text-xs text-[#586E75] max-w-sm">
              Official {orderType} message transmitted to <span className="font-semibold text-[#073642]">{toEmail}</span>.
            </p>
          </div>
        ) : (
          <form onSubmit={handleSendEmail} className="space-y-3 text-xs">
            {/* Recipient */}
            <div>
              <label className="block font-semibold text-[#073642] mb-1">Recipient Vendor Email *</label>
              <input
                type="email"
                required
                value={toEmail}
            ...
            {/* Subject */}
            <div>
              <label className="block font-semibold text-[#073642] mb-1">Subject *</label>
              <input
                type="text"
                required
                value={subject}
            ...
            {/* Body */}
            <div>
              <label className="block font-semibold text-[#073642] mb-1">Email Message Payload</label>
              <textarea
                rows={7}
                required
                value={bodyText}
                onChange={e => setBodyText(e.target.value)}
                className="w-full bg-[#EEE8D5] border border-[#D6D1B1] rounded-xl p-3 text-xs text-[#073642] font-mono focus:outline-none focus:border-emerald-500 leading-relaxed resize-none"
              />
            </div>

            {/* Attachments */}
            <div>
              <label className="block font-semibold text-[#073642] mb-1">Attachments</label>
              <div className="flex items-center gap-2">
                <label className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#EEE8D5] hover:bg-[#E4DDC7] border border-[#D6D1B1] text-[#073642] cursor-pointer text-xs font-semibold">
                  <Paperclip className="w-3.5 h-3.5 text-emerald-600" />
                  <span>Attach Document</span>
                  <input type="file" multiple onChange={handleFileChange} className="hidden" />
                </label>
                {attachedFiles.length > 0 && (
                  <span className="text-xs text-[#586E75]">{attachedFiles.length} file(s) attached</span>
                )}
              </div>
              ...
              {attachedFiles.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-2">
                  {attachedFiles.map((file, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-1.5 bg-[#EEE8D5] border border-[#D6D1B1] px-2.5 py-1 rounded-lg text-xs"
                    >
            ...
            <div className="bg-[#EEE8D5] p-3 rounded-xl border border-[#D6D1B1] space-y-1 text-[11px] text-[#586E75]">
              <span className="font-bold text-[#073642] block">Verified Procurement Footprint:</span>
