      ...
      <div className="bg-[white] w-full sm:max-w-xl sm:rounded-3xl rounded-t-3xl border border-[#e2e8f0] shadow-2xl flex flex-col max-h-[90vh] text-[#0f172a]">
        <div className="flex items-center justify-between border-b border-[#e2e8f0]/60 px-5 py-4 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-lg font-bold text-[#0f172a]">Edit Component Details</h3>
              <p className="text-xs text-[#64748b]">Category: {formData.category}</p>
            </div>
          </div>
            ...
            <div className="space-y-3 p-4 bg-[white] rounded-2xl border border-[#e2e8f0] shadow-xs">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5">
                <div>
                  ...
                  <p className="text-[11px] text-[#64748b] mt-0.5">Manage vendor associations. 2+ vendors enables <strong>Compare Companies</strong> AI engine.</p>
                </div>
              ...
              {selectedCompanies.length > 0 ? (
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {selectedCompanies.map((s, idx) => {
                    ...
                    return (
                      <span key={s.company_id} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white text-[#0f172a] border border-[#e2e8f0] text-xs font-bold shadow-xs">
                        ...
                        <button type="button" onClick={() => handleRemoveCompany(s.company_id)} className="p-0.5 rounded-full hover:bg-red-100 text-[#64748b] hover:text-red-700 transition-all cursor-pointer ml-1">
                          <X className="w-3 h-3" />
                        </button>
                      </span>
                    );
              ...
              {selectedCompanies.length > 0 && (
                <div className="space-y-2 mt-2 pt-2 border-t border-[#e2e8f0]/60">
                  <span className="text-[10px] uppercase font-bold tracking-wider text-[#64748b] block">Company Commercial Parameters:</span>
                      ...
                      return (
                        <div key={s.company_id} className="p-3 rounded-xl bg-[white] border border-[#e2e8f0] space-y-2">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                            <div>
                              <label className="block text-[10px] font-semibold text-[#64748b] mb-0.5">RFQ Price (Rs.)</label>
                            ...
                            <div>
                              <label className="block text-[10px] font-semibold text-[#64748b] mb-0.5">Lead Days</label>
                            ...
                            <div>
                              <label className="block text-[10px] font-semibold text-[#64748b] mb-0.5">Vendor SKU</label>
                              <input type="text" value={s.part_number_vendor}
                                onChange={e => handleUpdateMapping(s.company_id, { part_number_vendor: e.target.value })}
                                className="w-full bg-[white] border border-[#e2e8f0] rounded-lg px-2 py-1 text-xs font-mono text-[#0f172a] focus:outline-none focus:border-emerald-500" placeholder="OEM-SPEC" />
                            </div>
                          </div>
                        </div>
                      );
