...
export const EditComponentModal: React.FC<Props> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (selectedCompanies.length === 0) {
      setCompanyError('Please associate at least 1 company for this component.');
      return;
    }
            ...
            <div className="space-y-3 p-4 bg-[#FDF6E3] rounded-2xl border border-[#D6D1B1] shadow-xs">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5">
                <div>
                  ...
                  <p className="text-[11px] text-[#586E75] mt-0.5">Manage vendor associations. 2+ vendors enables <strong>Compare Companies</strong> AI engine.</p>
                </div>
              ...
              {selectedCompanies.length > 0 ? (
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {selectedCompanies.map((s, idx) => {
                    ...
                    return (
                      <span key={s.company_id} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white text-[#073642] border border-[#D6D1B1] text-xs font-bold shadow-xs">
                        ...
                        <button type="button" onClick={() => handleRemoveCompany(s.company_id)} className="p-0.5 rounded-full hover:bg-red-100 text-[#586E75] hover:text-red-700 transition-all cursor-pointer ml-1">
                          <X className="w-3 h-3" />
                        </button>
                      </span>
                    );
              ...
              {selectedCompanies.length > 0 && (
                <div className="space-y-2 mt-2 pt-2 border-t border-[#D6D1B1]/60">
                  <span className="text-[10px] uppercase font-bold tracking-wider text-[#586E75] block">Company Commercial Parameters:</span>
                  <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                    {selectedCompanies.map((s, idx) => {
                      const supp = companies.find(sup => sup.id === s.company_id);
                      return (
                        <div key={s.company_id} className="p-3 rounded-xl bg-[#EEE8D5] border border-[#D6D1B1] space-y-2">
